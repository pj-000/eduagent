...
def create_explore_agent(llm_config: AliyunLLMConfig) -> CustomizeAgent:
    """
    创建自主探索 Agent
    Agent 根据项目背景，自主决定搜索关键词和搜索方向，
    从互联网上寻找教育领域的创新功能点和创意灵感。
    """

    # 初始化搜索工具（均无需额外 API Key）
...
（列出所有参考链接）""",
    ...
    agent = CustomizeAgent(
        ...
        description="教育创新功能自主探索 Agent，根据项目背景自主联网搜索教育领域创新功能点",
        prompt_template=template,
        inputs=[
            {
                "name": "project_context",
                "type": "str",
                "description": "项目背景描述，包含已有能力和技术栈",
            },
            {
                "name": "explore_hint",
                "type": "str",
                "description": "探索方向提示，可为空让 Agent 完全自主",
            },
        ],
        outputs=[
            {
                "name": "explore_report",
                "type": "str",
                "description": "创新功能探索报告，包含可落地的功能点清单",
            },
        ],
...
def save_result(explore_hint: str, result: str) -> tuple[Path, Path]:
...
示例：
  ...
  python scripts/search_edu.py --no-save
    ...
    parser.add_argument(
        ...
        default="all",
        help="预设探索方向：all(全方位) / adaptive(自适应) / assessment(评测) / content(内容) / interaction(交互)",
    )
    parser.add_argument(
        "--hint",
        "-H",
    ...
    # 确定探索方向
    explore_hint = args.hint if args.hint else DEFAULT_EXPLORE_HINTS[args.focus]
    ...
    print(f"📐 探索方向：{args.focus if not args.hint else '自定义'}")
    print("-" * 60)

    agent = create_explore_agent(llm_config)
    ...
    # 保存结果（默认保存）
    if not args.no_save:
        json_path, md_path = save_result(explore_hint, result)
        print(f"\n💾 JSON 结果：{json_path}")
